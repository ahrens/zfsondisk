# Vdevs, Labels, and Boot Block {#sec:vdev}
...
blank space
(secion @sec:blankspace),
boot block header
(secion @sec:bootheader),
...
------------    --------------- ---------------------------    -------------------------------------------------
Version         ``version”      \small DATA_TYPE_UINT64        On disk format version.
                                                               Current value is “1” (5000?).
                                                               
Name            ``name”         \small DATA_TYPE_STRING        Name of the pool in which this vdev belongs.
                                                               
State           ``state”        \small DATA_TYPE_UINT64        State of this pool. The following table shows all
                                                               existing pool states. The @tbl:pool_states shows
                                                               all existing pool states.
                                                               
Tranaction      ``txg”          \small DATA_TYPE_UINT64        Transaction group number in which this label was
                                                               written to disk.
                                                               
Pool Guid       ``pool_guid”    \small DATA_TYPE_UINT64        Global unique identifier (guid) for the pool.
                                                               
Top Guid        ``top_guid”     \small DATA_TYPE_UINT64        Global unique identifier for the top-level vdev
                                                               of this subtree.
                                                               
Vdev Tree       ``vdev_tree”    \small DATA_TYPE_NVLIST        The vdev_tree is a nvlist structure which is used
...
**Name**               **Value**                         **Description**
--------------------   -------------------------------   -------------------------------------------------
``type”                \small DATA_TYPE_UINT64           The id is the index of this vdev in its parent's
                                                         children array.
                                                         
``guid”                \small DATA_TYPE_UINT64           Global Unique Identifier for this vdev_tree element.
                                                         
``path”                \small DATA_TYPE_STRING           Device path. Only used for leaf vdevs.
                                                         
``devid”               \small DATA_TYPE_STRING           Device ID for this vdev_tree element. Only used for
                                                         vdevs of type disk.
                                                         
 “metaslab_array”      \small DATA_TYPE_UINT64           Object number of an object containing an array of object
                                                         numbers. Each element of this array (ma[i]) is, in turn,
                                                         an object number of a space map for metaslab 'i'.
                                                         
“metaslab_shift”       \small DATA_TYPE_UINT64           Log base 2 of the metaslab size.
                                                         
 “ashift”              \small DATA_TYPE_UINT64           Log base 2 of the minimum allocatable unit for this
                                                         top level vdev. This is currently '10' for a RAIDz
                                                         configuration, `9' otherwise.
                                                         
“asize”                \small DATA_TYPE_UINT64           Amount of space that can be allocated from this top level
                                                         vdev
                                                         
 “children”            \small DATA_TYPE_NVLIST_ARRAY     Array of vdev_tree nvlists for each child of this
                                                         vdev_tree element.
--------------------   -------------------------------   --------------------------------------------------
...
ub_version:
  ~ The version field is used to identify the on-disk format in which this data is laid out.
    ...
    as the “version” element of the name/value pairs described in secion @sec:nvlist.
...
ub_guid_sum:
  ~ The ub_guid_sum is used to verify the availability of vdevs within a pool.
    ...
    (a vdev's guid is stored in the guid nvpair entry, see secion @sec:nvlist) it encounters.
...
ub_rootbp:
  ~ The ub_rootbp is a blkptr structure containing the location of the MOS.
    ...
    chapters @sec:dsl and @sec:blkptr respectively.
